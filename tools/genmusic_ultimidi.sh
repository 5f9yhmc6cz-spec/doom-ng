...
set -e
# Soundfont: explicit SF= wins; else auto-detect a General-MIDI .sf2 (Linux distro paths
# first, then macOS/Homebrew) -- same as tools/genmusic.sh. Override with SF=/path.
if [ -z "${SF:-}" ]; then
  for _c in \
