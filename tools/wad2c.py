...
NSHIFT = 16  # horizontal sub-tile phases per texture: 1-TEXEL granularity (was 8/2-texel). Textiles+ramps x2 in C-ROM -- funded by the symmetry harvest. Visibly sharper wall crops.
...
def _hudfix_run():
    ...
    def compose(face_lump):
        ...
        if face_lump: blit(face_lump, 148, 0, opaque=0)   # MASK the face (the author's preferred "first attempt"): its transparent pixels show STBAR's natural dark-grey (30,30,30) recessed slot, NOT opaque-black and NOT the wider slot-fill box that read as "cropped in too tight"
        return img, dmask
