...
# Running ONLY --ramps silently leaves stale textures (the footgun that hid the texture-opacity
# hole-fix for a whole debug session). rom-vs runs BOTH passes so textures and ramps stay in sync.
WAD ?= doom1.wad
