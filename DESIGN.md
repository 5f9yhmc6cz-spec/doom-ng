...
*Design notes — draft. This is a living document, subject to revision.*
...
"misuse the sprite engine as a pixel renderer" dies here. This design avoids that.
...
Get fps by running `make gngeo` with a display, or build a dedicated bench harness.
...
3. [BUILDS] SCB backend: `neogeo/main.c` drives `render_world`'s DrawList onto real SCBs
   ...
   Verified visually in gngeo with a display, since it won't run headless.
