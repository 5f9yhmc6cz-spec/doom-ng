...
#define CEIL_DIM 8   /* (cmap ON) ceiling ramp brightness, /16. Lower = darker, blends harder into the murk (the author 2026-06-23: "too light end to end"). 16 = full bright (= cmap OFF). */
...
#define VS_NDD 33
#define VS_NDC 11
static int g_dd=14, g_dci=4;   /* (the author default DD[14]=750 / DC[4]=5) draw distance (DD[] idx) + per-column DEPTH CAP (DC[] idx -> g_dcap). Default DD[11]=600 (playable) / DC[5]=6. (Was DD[max]=32000 "draw everything"; moved to a playable default now that perf is mature + the node far-cull makes dd actually prune the walk.) */
...
static int g_actddi=3;         /* (41) ACTOR draw distance = DD[] idx; default DD[3]=200 (the author's pick) -- monsters/props cull at 200u, independently of the wall dd. Dial UP (toward 32000=off) for more visible actors, DOWN for denser-map perf. */
...
static int g_zonal=0;          /* ZONAL flats: per-row visplane (correct floor/ceil flat per depth band). DEFAULT 0 (off/blanket, the author 2026-06-23): the real-flat zonal visplane is parked behind the toggle. 1=zonal; 2/3=bias up/down; 4=round (no bias); 5=round+grey-wins; 6=sub-tile. Shuttle param 4. Needs gen=0. */
static int g_generic=1;        /* GENERIC mode: synthetic floor/ceiling LUT + sky (the blanket look) instead of real flats; shuttle param 5. DEFAULT 1 (the author 2026-06-23) = the synthetic blanket is the ship default; set gen=0 to enable real flats + the zonal visplane (param 4). */
#define NFOGBIAS 10
static const unsigned char FOGBIAS[NFOGBIAS]={15,20,25,30,35,40,48,56,66,78};   /* (6) WALL-FOG CURVE: g_fog0 (the near band edge) as a % of g_fog1 (the far edge, set by fogext). Low=heavy FRONT bias (mid-fog onsets close); idx5=40=NEUTRAL (the old 2:5 = 300/750); high=REAR bias (bright most of the way, dark only at the far edge). 10 steps (the author 2026-06-23: finer range). */
...
static int g_weapon=1, g_pweap=-1; /* current weapon (0=fist..7=bfg; default pistol=1) + last-drawn (for change-only redraw). GAMEPLAY P cycles it now (2026-06-23, 'liberated from the menu'); debug case 7 kept off-grid. */
...
#define NMURKMIN 30
static int g_murkmin=5;    /* (8, the author default MURKMIN[5]=250) far-cull FLOOR: min g_murk_eff under load. Default idx10=500 -> with the node far-cull this prunes the WALK to ~500u under budget pressure (perf). Last idx (-1) = OFF (draw everything). */
...
static int g_vpw=0, g_vph=5;   /* (18) FLOOR (fmrk) + (19) CEILING (cmrk) murk: # of FAR rows (toward the horizon) faded to the murk shades. SYNTHETIC planes (gen=1). g_vph=5 default -> the COLMAP CEILING depth fade shows (kills the perspective banding, the author 2026-06-23); cmrk(19) tunes it. fmrk(18) floor still 0=off by default. */
#define NVP 10                 /* murk levels 0..9 (the author 2026-06-23, was 0..5); fmrk/cmrk clamp to the band's row count (floor 5 / ceiling 7) so the high end darkens the whole band */
...
static int g_occl=1;      /* actor occlusion vs walls (M2): barrels/enemies hidden behind nearer solid walls. HARDWIRED ON 2026-06-23 (the author: enemies always need occluding) -> param 12 freed for the perfP toggle. */
...
static int g_maxact=8;        /* (42) runtime nearest-N actor cap (1..MAXACT array size); default 8 (the author's pick). Dial UP for more simultaneous actors, DOWN for monster-heavy-view perf */
...
static const unsigned char FCLOD_F[NFCLOD]={5,4,3,2,1,0,0,0,0,0}, FCLOD_C[NFCLOD]={7,6,5,4,3,2,1,0,0,0};   /* 1 increment = 1 row (the author): flod 0..5 = floor 5..0 rows, clod 0..7 = ceiling 7..0 rows; steps past 0 stay parked */
...
/* OPAQUE-CORNER cap (TB32o) palette slots. The HW palette RAM is fully booked in VS gameplay
   ...
   PERMANENTLY-free slot. We reserve the TOP TWO wall/flat slots (242,243): the all-E1 wall+flat
   ...
   to a texture, and we re-assert the cap palettes after every per-map upload. Caps OFF -> the ceiling
...
static const short BXCAP[]={8,12,16,20,24,28,32,40,48,64,68,72,76,80,84,88,92,96,100,104,108,112,116,120,124,128,160,192,256,384,2048};   /* (44) PHASE 2 WALK BUDGET: max BSP node-box tests (bx=g_bbox_n) per frame. FINE low end (8..48) + FINE 64..128 in steps of 4 (the author 2026-06-23) to find where the walk breaks + eases into the murk. The walk halts when bx hits this; front-to-back order means the unvisited stack is the FAR subtrees -> they drop first (depth-ordered, ~ a node-count dd). Bounds the WORST-CASE walk on node-dense/stair maps. Last=2048=OFF. */
...
static int g_opensky=1;   /* SKY-IN-OPENING: draw sky through windows/openings whose BACK ceiling is sky (#42). HARDWIRED ON 2026-06-23 (the author) -> param 14 freed for the cmap (colmap-ceiling) toggle. */
...
static const unsigned char GOVFPS[]={0,1,2,3,4,5,6,10,12,15,30,60};   /* (46) gov dial = TARGET FPS (0=off). All divide 60 evenly -> the governor's vblank target = 60/fps (1fps->60 vbl .. 60fps->1 vbl). Sweep it to read the honest fps<->view-distance curve; the low end (1-5fps) shows how much geometry survives when you stop demanding speed. */
...
#define NRAD 9
static int g_radial=0;   /* DEPRECATED 2026-06-23 (the author: param 23 reused for 'fcrv' fog curve). Stays 0 -> the radial far-cull paths below (1035/1118/1119/1131/1443) are inert. (Was: RADIAL far-cull strength curve, 1..8 = per-column euclidean pull-in at eccentric angles.) */
static int g_vmap=3;     /* (24, the author default 3) VERTICAL MAP: 0 = ORIGINAL stretch (whole texture to wall height, no tiling/peg); 1 = V companion everywhere (1:1 world-scale + vertical tiling + DOOM peg, AFFINE depth); 2 = SELECTIVE (companion only for single-copy posters/signs, stretch for tiling walls); 3 = companion everywhere + PERSPECTIVE-CORRECT depth (the wf 1/z-linear depth -> the 1:1 vertical scale tiles at the true rate, so brick/panel courses stay level on angled walls; twin of hmap=3). Default 2 (the author). */
...
static int g_bandclip=1;   /* (39) bclp: VERTICAL BAND-REJECT -- skip a seg whose projected height misses every open column's surviving sliver (the near-window chug fix). Default ON; A/B toggle. */
static int g_dpri=1;       /* (40) dpri: DEPTH-PRIORITISED BUDGET -- record every strip within the horizon, then emit the NEAREST 'bud' by DEPTH (a depth histogram) instead of halting the walk at the budget. Fixes near walls dropping when the BSP visits them after the budget fills (the dense-courtyard near-wall wink). Default ON; A/B toggle (off = old walk-order halt). */
static int s_px=0x7fffffff, s_py=0, s_ang=0, s_aph=0;   /* static-skip signature: last-RENDERED camera + (g_anim>>3) actor phase */
static int g_skipcnt=0;   /* running count of SKIPPED frames -> climbs ~60/sec while idle (Schrodinger-free proof the skip fires); frozen while moving */
#define FCN 32
static unsigned char g_fcost[FCN]; static int g_fci=0;   /* frame-cost SCOPE ring: per-frame vblank cost (cad), logged EVERY frame incl. while moving. The live fps= readout freezes to 60 when you stop (HUD locks movement = Schrodinger); this ring keeps the MOVING history -> read it after a move to see the heavy frames, then watch the tail settle to 1 as the static-skip kicks in. */
...
#define NNCLIP 18
static int g_nclip=1;   /* (36) NEAR-CLIP dial: idx into NCLIP[]; the author default idx1=8 (2026-06-23, was idx2=16). Push the near plane OUT to clip the huge near WALLS (perf probe: do the widest near strips / screen-eccentric edges dominate? does drawing slightly deeper save time?). 0 = degenerate near vtx project to centre + back-face cull (safe). */
...
/* PERF (crib from Doom8088 FLAT-pad discipline): per-SCB-slot real-tile HIGH-WATER. The shrunk-strip
   vshrink over-read (gngeo) only reads stale REAL tiles past 'cot', so we keep the invariant "tiles
...
static void vs_strip_emit(int spr,int c,int y0,int y1,int tex,int tcol,int d,int dyt,int dyb,int yt0,int voff,int poster){
    ...
    /* OPAQUE-CORNER cap (TB32o): the wall strip stays PLAIN -- force the in-strip cap tiles OFF so y0/y1/
       ...
       Latch which edges WOULD have been capped (same gate) + their slope/flip so we can lay the overlay
...
static void vs_lut(int ang){
    ...
    for(int k=0;k<FLOORLUT_COLS;k++){
        ...
        fbase=vs_flatres(vs_ffl[wc],a64,sph,1,fbase0,13,&fpal);   /* nearest-seg FRONT floor flat; un-latched (0xFF) -> synthetic (vs_flatres). TAKE NO ACTION on unresolved cells (the author 2026-06-23: the RHS-wrong was incomplete-BSP geometry, not a fallback choice -- camera flat mis-painted it, murk darkened half the screen; original synthetic is the honest baseline). */
    ...
    for(int c=ca;c<=cb;c++, f+=df){
        ...
        if(g_hmap==3 || g_vmap==3){   /* PHASE 5: advance the subdivide-affine 1/z interpolant EVERY column -> it stays locked to SCREEN-X (not the visible-column count), so a wall peeking past a near occluder doesn't swim (the desync the verify caught). */
            if(psub<=0){   /* resample the EXACT 1/z fraction (one 32-bit divide) only every g_pstepv cols -- was a divide EVERY col = the hidden per-column tax, un-throttled by any dial */
                ...
                else pwfs=0;                                               /* PSTEP=1, OR no next col in this seg (1-col / last-col): SKIP the lookahead divide -> a sliver / narrow-col-mode seg pays 1 divide not 2 (the author: don't trash the benefit at ~20 cols) */
        ...
        if(g_zonal && !g_generic){   /* VISPLANE STAMP -- only when the LUT actually READS it (gen=1 uses the synthetic blanket, so the stamp was pure wasted memory-bound work = the perf win). */
            ...
            int ffl_s=(fr>=4)?g_cam_ffl:ve_ffl[s];   /* CAMERA-SECTOR DEFAULT: near-band floor edge (fr==4 = your level floor) takes the camera's own sector flat; a higher edge (fr<4 = a distinct stepped/lower floor) keeps the seg's own flat. Fixes the nearest-seg-is-a-far-wall mis-latch without the blanket-flip regression. */
            ...
            { int sd=(int)(((long)dx*vs_fsn-(long)dy*vs_fcs)>>8);          /* FRUSTUM + OCCLUSION pre-cull (#actor): project the centre, then reject off-screen / fully-behind-wall actors BEFORE they eat a nearest-N slot or pay atan2 + vs_billboard's full projection. */
              ...
              if(g_occl){ int cls=ve_thc[i]; if(cls<0||cls>=NCLS)cls=0; int rfs=rf>512?512:rf;   /* OCCLUSION pre-cull: skip an actor whose every drawn column is behind a nearer SOLID wall, before atan2 + vs_billboard. */
                ...
                if(lcx<=rcx){                                              /* some on-screen extent (fully off-screen actors fall through to vs_billboard, which emits ~0 cheaply) */
                    int rl=((unsigned short)lcx*(unsigned short)g_colrcp)>>16; if(rl>g_ncol-1)rl=g_ncol-1;   /* clamp to [0,319] mirrors vs_billboard's own wcl/wcr clamp, so we sample exactly the render-cols it would */
...
static unsigned char deadstate(int i){ int c=ve_thc[i]; if(c<0||c>=NCLS)c=0; return (ve_thc[i]!=0 && SPRCLASS[c].corpse)?2:0; }   /* 2 = leave a CORPSE (baddie with a death sprite); 0 = gone (barrel / no corpse frame). Items never reach here (not shootable). */
...
/* ===== STACK HIGH-WATER PROFILER. Work RAM is the binding constraint: statics end at `_end` (~0x10D6C4)
   and the m68k supervisor stack grows DOWN from 0x10F300, so the WHOLE stack budget is ~7.2KB. At boot we
   paint the free region with a sentinel byte; the deepest the stack ever reaches overwrites it, and the
   debug HUD scans for the high-water. `stk` = peak bytes the stack has used, `fr` = bytes still free between
   the high-water and the statics (fr -> 0 means the stack is about to corrupt .bss = the crash we're hunting). ===== */
...
static void stack_paint(void){
    ...
    for(volatile unsigned char *p=(unsigned char*)_end; p<(unsigned char*)(sp-256); p++) *p=STK_SENT;   /* fill below us, 256B clear of the live frame */
}
...
static const cfgpreset_t CFGP[]={
  /*               dd col bud mmin ea dc ncl fl cl vw vh gn zn nc bc oc sk hm vm cp rd sm hw  (23 fields). Updated 2026-06-22: DEFAULT=the author's boot config (dd750/mn250/dc7/nclp16/hmap3/vmap3); perf presets carry ease=4 (adaptive far-cull) + FLOOR drops hmap0/vmap0 (no perspective divide). */
...
static void cfg_apply(int i){ const cfgpreset_t *P=&CFGP[i];
  ...
  g_redraw=1; }
/* the author's FINAL-TESTING defaults (2026-06-23): the SINGLE source for the boot config + the R reset.
...
int main(void){
    vs_defaults();                  /* boot: the author's final-testing config (overrides the decl initialisers) */
    ...
    { int s=1; int TR=TITLE_ROWS;                            /* draw ALL 14 rows: the title is now baked NATIVE + vertically centred (wad2c.py) -> no stretch, no palette-merge glitch, so no row drop. 14 rows = 224px fills the active area; the baked letterbox shows the full title un-cropped (the author 2026-06-23). */
    ...
    {   /* VERTICAL SLICE -- never returns. Free movement in one room, live raycast walls. */
        ...
        for(;;){
            ...
            if(g_props) for(int i=0;i<ve_nth;i++){   /* ITEM PICKUP: walk over an armour/ammo billboard -> it vanishes + the DSITEMUP chime (SND 5). HUD counters are baked-static, so it's collect+chime only. */
                if(g_thalive[i]!=1 || ve_thc[i]<CLS_ITEM0) continue;   /* alive items (cls>=4) only -- skip barrels/baddies/corpses */
            ...
            if(!g_dbg && (dx||dy||(in&(CNT_LEFT|CNT_RIGHT)))) g_bobc++;   /* advance the gun-bob while walking OR TURNING (the author: bob on rotation too); holds steady only when fully still (and while debugging) */
            ...
            { static u8 pin=0; u8 pr=in & ~pin; pin=in;
              ...
              if(pr&CNT_B){ if(!g_dbg){ g_weapon=(g_weapon+1)%GUNHAND_N; }   /* GAMEPLAY P: cycle WEAPONS (2026-06-23, 'liberated from the menu'). The perf-preset cycle that used to live here is deprecated; presets still reachable via debug param 26 (cf). */
              ...
              { int dd=(pr&CNT_D)?1:((pr&CNT_C)?-1:0);      /* B: value UP / N: value DOWN (bidirectional, wraps) */
                if(dd) switch(g_sel){
                  ...
                  case 39:g_bandclip=!g_bandclip; break;                                          /* bclp: VERTICAL BAND-REJECT on/off -- A/B the near-window chug fix (watch sg/bc + window edges for popping) */
                  case 40:g_dpri=!g_dpri; g_redraw=1; break;                                      /* dpri: DEPTH-PRIORITISED BUDGET on/off -- A/B the dense-courtyard near-wall wink (on = keep the nearest bud by depth; off = old walk-order budget halt) */
                  ...
                  case 27: vs_defaults(); vs_ceil_pal(); vs_floor_pal(); g_redraw=1; break;   /* RESET to the author's final-testing defaults (vs_defaults = the single boot+reset source); re-scale the colmap floor/ceiling banks. */
                  ...
                  case 37:g_bspviz=!g_bspviz; g_bspstep=0; g_redraw=1; break;                       /* bspv: BSP TRAVERSAL VIZ -- depth-coloured walls revealed front-to-back, slowly, so you watch the live BSP walk */
                  case 38:g_hmap=(g_hmap+4+dd)%4; g_redraw=1; break;                               /* HMAP horizontal wall-U: 0=screen-mapped / 1=affine (swims) / 2=selective (posters only) / 3=PERSPECTIVE-CORRECT (glued + swim-free, ~1 divide/col). Separate from vmap(24) again (the author 2026-06-23). */
            ...
            if(g_dbg){ char tk[NSEL][12]; char ln[44];    /* HUD render gated on g_dbg (P toggles). ALL params laid out -> '>' caret on the P-selected one; screenshot the whole recipe at once. */
              ...
              if(g_bench){ if(g_fb_mode==1) snprintf(ln,sizeof ln,"FB %dvbl/16 ~%dk ~%dfps",g_bn_fa,(g_bn_fa*125)/10,g_bn_fa?960/g_bn_fa:0);   /* FB cost: vblanks for 16 frames -> ~cyc/frame (k) + fps */
                           else snprintf(ln,sizeof ln,"bn fa=%d em=%d fl=%d pj=%d ",g_bn_fa,g_bn_fa-g_bn_fe,g_bn_fa-g_bn_fp,g_bn_fe-g_bn_fb); ng_text(2,15,1,ln); }   /* BENCH (bnch=2): per-stage cost in 16x-vblank units -- fa=FULL, em=emit(SCB writes), fl=flats(floor/ceil LUT), pj=proj(BSP+projection). Lower=cheaper; see which dominates as you crank dials. */
              /* SCOPE digit-strip (row 15) removed from the default debug view (the author: "rather not draw the long string of numbers"). The ring still feeds w= (recent worst) on the fps line; re-add the strip behind a toggle if the timeline is wanted again. */
#ifdef VS_DIAG
