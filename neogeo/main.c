...
static int g_opensky=1;   /* (14, 'sky') SKY-IN-OPENING: draw sky through windows/openings whose BACK ceiling is sky (#42). Dial restored 2026-06-28 (cmap freed slot 14). */
...
/* TURN SENSITIVITY (param 53 'turn'): angle-units/frame x16 (the angle is 0..255 = 360deg, 1 unit = 1.4deg).
   ...
   "halved" finer option the author wanted to try (smoother, slower); TURN[2..9] = eight LESS-fine (faster) steps in
...
static int g_rmk=2, g_rmksh=1;   /* (52, ex-asm) rm16 MODE-2 draw:march RATIO K (power-of-2: 2 or 4 -> shift). MARCH g_ncol/K occlusion slots, DRAW g_ncol cols, Q1-interpolating the K-1 fill cols between marched anchors. K=4 @ col64 = "draw 64 from a source of 16" (the author). Only meaningful when rm16==2 (interp). */
...
static void vs_defaults(void){
    g_dd=11; g_dci=3; g_dcap=4; g_ncoli=1; g_murkmin=3; g_fogext=6; g_floorlod=0; g_ceillod=1;   /* DEFAULTS (the author 2026-06-28): dd600 (DD[11]), dc4 (DC[3], the -6% see-through cliff), col32, clod1. */
    ...
    g_zonal=0; g_generic=1; g_occl=1; g_capmode=0; g_opensky=1; g_radial=0; g_hclip=0; g_headbob=2; g_turni=4;   /* g_capmode pinned 0 (caps dormant); hbob2 + turn4 (the author 2026-06-28). */
    ...
    g_bandclip=1; g_dpri=1; g_budgeti=23; g_actddi=7; g_maxact=4; g_bxcapi=18; g_pstepi=0;   /* DEFAULTS (the author 2026-06-28): add400 (DD[7]), mxa4, bxc100 (BXCAP[18]). pstepi=0 (exact 1/z, moot at hmap2). bud300/dpri kept. */
 ...
 * a marker word to it. We auto-skip the title/menu, drive the camera through a canned movement
 * script (the REAL movement/collision code -> deterministic + config-independent), reset to spawn
 * between configs so every config sees the identical scene, and bracket each vs_render with
            ...
            { static u8 pin=0; u8 pr=in & ~pin; pin=in;
              ...
              { int dd=(pr&CNT_D)?1:((pr&CNT_C)?-1:0);      /* B: value UP / N: value DOWN (bidirectional, wraps) */
                if(dd) switch(g_sel){
                  ...
                  /* zon (4) + gen (5) toggles DISABLED for now (the author 2026-06-28). g_zonal pinned 0 (blanket), g_generic pinned 1 (synthetic flats) -- the ship defaults; off the menu. */
                  ...
                  /* rmk (52) toggle REMOVED (the author: higher ratios = exponential gaps). g_rmk pinned 2 (the clean 2:1 fold); the K:1 code stays but only ever marches 2:1. */
