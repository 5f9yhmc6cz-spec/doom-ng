...
# --- BANKED GEOMETRY P2 (all-Ultimate): declare a 7th region so gngeo maps 202-p2 at
# 0x200000. 202-p2 = the live-BSP geometry blob (3 banks now; auto-sized from the built file).
P2MB=1; [ -f "$SRCDIR/build/rom/202-p2.p2" ] && P2MB=$(( ($(stat -f%z "$SRCDIR/build/rom/202-p2.p2")+1048575)/1048576 )); [ "$P2MB" -lt 1 ] && P2MB=1
